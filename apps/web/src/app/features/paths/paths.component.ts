...
@Component({
  ...
  template: `
    <header class="page-header">
      ...
      <span>Java professionnel est complet : 22 laboratoires, un projet portfolio et un défi final adaptatif.</span>
    </header>
    @if (state$ | async; as state) {
      @if (state.status === 'loading') {
        ...
        <section class="path-progress java-progress" aria-label="Progression Java">
          <div><p>Parcours principal V2.7</p><h2>Java professionnel</h2><span>{{ state.java.completedLabs }} / {{ state.java.totalLabs }} étapes validées</span></div>
          <div class="progress-track" role="progressbar" aria-label="Progression du parcours Java" [attr.aria-valuenow]="state.java.progressPercent" aria-valuemin="0" aria-valuemax="100"><span [style.width.%]="state.java.progressPercent"></span></div>
          <small>{{ state.java.progressPercent }} % · Prochaine étape : {{ state.java.nextLabCode ?? 'parcours terminé' }}</small>
        </section>
        <section class="path-grid java-grid" aria-label="Parcours Java professionnel">
          @for (lab of state.java.labs; track lab.code; let index = $index) {
            @if (lab.state !== 'LOCKED') {
        ...
        <section class="path-progress" aria-label="Progression Python">
        ...
        <section class="path-grid python-grid" aria-label="Laboratoires Python professionnel">
          @for (lab of state.python.labs; track lab.code; let index = $index) {
            @if (lab.state !== 'LOCKED') {
              ...
              <article class="lab-card locked"><span class="number">{{ activityMarker(lab.activityType, index) }}</span><div><p>{{ lab.code }} · VERROUILLÉ</p><h2>{{ lab.title }}</h2><small>Termine {{ lab.prerequisites.join(', ') }}</small></div><span class="arrow" aria-hidden="true">🔒</span></article>
            }
          }
        </section>
  ...
  styles: [`
    ...
    .lab-card.locked{filter:saturate(.35);opacity:.55}.lab-card.locked:hover{border-color:var(--border);transform:none}.lab-card.done{border-color:#347865}.lab-card.project{box-shadow:inset 0 3px #e7b85c}.lab-card.project .number{background:#3d3020;color:#ffd98b}.lab-card.challenge{box-shadow:inset 0 3px #e5688a}.lab-card.challenge .number{background:#422136;color:#ffb3c7}
...
export class PathsComponent {
  ...
  readonly state$ = forkJoin({ labs: this.labs.listLabs(), paths: this.labs.listPaths(), java: this.labs.pathProgress('JAVA'), python: this.labs.pathProgress('PYTHON') }).pipe(
    map(({ labs, paths, java, python }) => ({ status: 'loaded' as const, labs, paths, java, python })),
    startWith({ status: 'loading' as const, labs: [], paths: [], java: this.emptyProgress('JAVA'), python: this.emptyProgress('PYTHON') }),
    catchError(() => of({ status: 'error' as const, labs: [], paths: [], java: this.emptyProgress('JAVA'), python: this.emptyProgress('PYTHON') }))
  );
  ...
  private emptyProgress(pathCode: string) { return { pathCode, totalLabs: 0, completedLabs: 0, progressPercent: 0, nextLabCode: null, labs: [] }; }
}
